		...
		switch(menu_type){
			...
			case 3:
			  ...
			  col+='<div class="banner">';
				...
				col+='<h4 class="stic-bold">'+val.item_name+'</h4>';
...
fillBookingTabs = function(element, index){
	...
	if(settings = AppSettings()){		
		$.each( settings.booking_tabs, function( key, val ) {
			...
			html+='<ons-carousel-item class="'+ is_selected +'" onclick="SetBookingTab(' + clickFormat(key+"|"+val.tab) + ');" >'+val.label+'<ons-ripple color="#eee" background="#ffffff"></ons-ripple></ons-carousel-item>';
			var newItem = ons.createElement(html);
		    list.appendChild(newItem);
		    html='';
		});
		...
		html+='<ons-list-item class="stic-order-history" modifier="full_list list_style1">';
			...
			html +='<ons-ripple modifier="material"></ons-ripple>';
				...
				html+='<ons-row class="review">';
					...
					if (empty(val.rating)) {
						html+='<div class="raty-medium"><span class="color_green">'+t("Review my order")+'</span></div>';
					} else {
...
setOrderHistory = function(element, data){		
	...
	html+='<p class="small print_trans">'+t("Status")+': '+ data.status +'</p>';
			...
			html+='<ons-carousel-item>';
				...
				if (key == 0 || key == "0") {
					html+='<div class="stic-walkthrough is_rtl">';
						html +='<h3 class="trn">'+ t('Stay home & Order from phone') +'</h3>'; 
						html +='<p class="small trn">'+ t('Order your favourite dishes from our delicious menu with a few clicks')+'</p>'; 
						html +='<p class="trn">'+ t('Swipe Right') +'</p>'; 
					html+='</div>';
		   		}

		   		if (key == 1 || key == "1") {
					html+='<div class="stic-walkthrough is_rtl">';
						html +='<h3 class="trn">'+ t('We prepare Clean & Safe') +'</h3>'; 
						html +='<p class="small trn">'+ t('We monitor our kitchen with CCTV surveillance, for healthy and safe cooking. You are guaranteed safe food!') +'</p>'; 
					html+='</div>';
		   		}
		   		...
		   		if (key == 2 || key == "2") {
					html+='<div class="stic-walkthrough is_rtl">';
						html+='<div>';
							...
							html +='<p class="small trn">'+ t('Get it delivered. We could even leave it on your doorhandle. You don\'t even have to get up off the couch') +'</p>'; 
